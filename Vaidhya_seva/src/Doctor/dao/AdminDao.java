...
public class AdminDao {
    ...
    // Main menu for doctor operations
            ...
            while (true) {
                ...
                try {
                    switch (ch) {
                        ...
                        case 4 -> suggestAlternateDoctor(conn);
                        case 5 -> { return; }
    ...
    public static void insertDoctor(Connection conn) throws SQLException {
        ...
        String specialization = JOptionPane.showInputDialog("Enter Specializzation:");
        ...
        String sql = "INSERT INTO doctor (DoctorId, DoctorName, DateOfBirth, DateOfJoining, Gender, Qualification, " +
                     "Specializzation, YearsOfExperience, Street, Location, City, State, Pincode, ContactNumber, Emailid) " +
                     "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    sb.append("ID: ").append(rs.getString("DoctorId")).append("\n");
                    ...
                    sb.append("Specialization: ").append(rs.getString("Specializzation")).append("\n");
                    sb.append("Experience: ").append(rs.getInt("YearsOfExperience")).append(" years\n");
    ...
    public static void updateDoctor(Connection conn) throws SQLException {
        ...
        String[] fields = {
            "DoctorName", "DateOfBirth", "DateOfJoining", "Gender",
            "Qualification", "Specializzation", "YearsOfExperience",
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            JOptionPane.showMessageDialog(null,
                    rows > 0 ? "Doctor updated successfully." : "Doctor not found or update failed.");
        }
    }
    ...
    public static void deleteDoctor(Connection conn) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            int rows = stmt.executeUpdate();
            JOptionPane.showMessageDialog(null,
                    rows > 0 ? "Doctor deleted successfully." : "Doctor not found.");
        }
    }

    // Suggest alternate doctor if assigned doctor is on leave
    public static void suggestAlternateDoctor(Connection conn) {
        String patientId = JOptionPane.showInputDialog("Enter Patient ID:");
        if (patientId == null || patientId.trim().isEmpty()) return;

        // Get patient details
        PatientBean pb = PatientDao.checkPatient(AdminDao.getCon(), patientId);
        if (pb == null) {
            JOptionPane.showMessageDialog(null, "Patient not found.");
            return;
        }

        String assignedDoctorId = pb.getDoctorID();

        // Step 1: Get all doctors
        ArrayList<DoctorBean> allDoctors = DoctorDao.getAllDoctors();

        // Step 2: Get all doctors on leave today
        ArrayList<LeaveBean> leavesToday = new ArrayList<>();
        String sql = "SELECT * FROM `leave` WHERE CURDATE() BETWEEN Leave_From AND Leave_To AND Status = 1";
        try (Connection con = ReporterDao.getCon(); PreparedStatement ps = con.prepareStatement(sql); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
        ...
        for(LeaveBean lb:leavesToday)
        {
        if(pb.getDoctorID().equals(lb.getDoctorID()))
        {
        ...
        {
        System.out.println("Doctor available Confirm appointment");	
        }
        }  
        	// Step 3: Check if assigned doctor is on leave
        boolean assignedOnLeave = leavesToday.stream()
        ...
        // Step 4: Suggest alternate doctors
        StringBuilder sb = new StringBuilder("Assigned doctor is on leave.\nSuggested available doctors:\n");

        for (DoctorBean db : allDoctors) {
            boolean onLeave = leavesToday.stream()
                                         .anyMatch(lb -> lb.getDoctorID().equals(db.getDoctorID()));
            if (!onLeave) {
                sb.append("Doctor ID: ").append(db.getDoctorID())
                  .append(", Name: ").append(db.getDoctorName())
                  .append(", Specialization: ").append(db.getSpecialization())
                  .append("\n");
            }
        }
